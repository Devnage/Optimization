...
def main() -> None:
    ...
    quasiNewtonBFGS(f, initVector, errorTol, maxIter)

def quasiNewtonBFGS(f, initVector: np.ndarray, errorTol: float = 1e-6, maxIter: int = 100) -> None:
    table = []
    ...
    for k in range(maxIter + 1):
        ...
        s_k = newVector - currVector
        y_k = new_g_k - g_k

        denom = (s_k.T @ y_k)
        term1 = ((np.outer(H_k @ y_k, s_k)) + (np.outer(s_k, y_k) @ H_k)) / denom
        term2 = 1 + (((y_k.T @ H_k) @ y_k) / (denom))
        term3 = (np.outer(s_k, s_k)) / (denom)

        H_k = H_k - term1 + ((term2) * (term3))
        g_k = new_g_k
